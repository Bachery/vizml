...
c_aggregation_functions = [
    ...
    'has',
    'only_one',
    'all',          # 输入的保存特征的列表长度是25，对于维度数量小于25的数据集，all都会是false
    'percentage',   # 输入的保存特征的列表长度是25，导致比例不对
]
...
def aggregate_boolean_features(v, feature_name):
    ...
    try:
        ...
        r['{}-agg-all'.format(feature_name)] = all(v)           # v的长度是25，导致维度数量小于25的all都会是false
    except Exception as e:
        print(feature_name, e, len(v), v)

    r['{}-agg-percentage'.format(feature_name)] = sum(v) / len(v) if len(v) else None   # v的长度是25，导致比例不对
...
def aggregate_numeric_features(v, feature_name):
    ...
    r['{}-agg-avg_abs_dev'.format(feature_name)]        = np.mean(np.absolute(v - sample_mean))
    r['{}-agg-med_abs_dev'.format(feature_name)]        = np.median(np.absolute(v - sample_median))
    r['{}-agg-coeff_var'.format(feature_name)]          = (sample_mean / sample_var) if sample_var else None
    r['{}-agg-min'.format(feature_name)]                = sample_min
    r['{}-agg-max'.format(feature_name)]                = sample_max
    r['{}-agg-range'.format(feature_name)]              = sample_max - sample_min if (sample_max and sample_min) else None
    r['{}-agg-normalized_range'.format(feature_name)]   = (sample_max - sample_min) / sample_mean \
                                                            if (sample_max and sample_min and sample_mean) else None
