...
# This script pulls the models you saved in paper_tasks.py
# evaluates them on the specified file, and then prints out the
# output/saves to json

# contains the dataset_indices from the agg features.csv file
dataset_indices = [
    0,
    1,
    2,
...
# make sure that the mappings are correct, so that you print out the
# proper results!
def main():
    model_names = ['best_model_1.X', 'best_model_2.X']
    output_dims = [2, 3]
    mapping = [{0: 'bar', 1: 'line'}, {0: 'bar', 1: 'line', 2: 'scatter'}]
    output_file_names = [
        'task_1_nn_ground_truth.json',
        'task_2_nn_ground_truth.json']

    assert len(model_names) == len(output_dims) == len(
        mapping) == len(output_file_names)
    ...
    for i in range(len(model_names)):
        parameters = {
            ...
            'model_prefix': model_names[i],
            'output_dim': output_dims[i],
        }
        print('loaded dimensions are', X.shape)
        evaluate.print_predictions(
            X,
            fids,
            parameters,
            mapping[i],
